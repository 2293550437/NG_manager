app.controller('foodsController',function ($scope, $controller, foodsService) {
    $controller('baseController',{$scope:$scope});


    $scope.findId1=function(id){
      $scope.foodId=id;
      window.location.href="foods_edit.html";
    };

    //根据id查询单个对象
    $scope.findOne=function(id){
        foodsService.findOne(id).success(function (response) {
    ...
    $scope.findMenuClassList=function () {
        foodsService.findMenuList().success(function (response) {
            ...
            for (var i=0;i<response.length;i++){
                $scope.menuClassList[response.meclassId]=response[i].name;
